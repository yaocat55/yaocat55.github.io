...
flowchart TD
    ...
    subgraph LEVEL_DECISION ["日志级别选择决策树"]
        ...
        Q1 -->|是| TRACE[TRACE<br/>最细粒度<br/>方法内变量值/循环体]
        Q1 -->|否| Q2{"需要定位复杂Bug<br/>或跟踪完整调用链 ?"}
        Q2 -->|是| DEBUG[DEBUG<br/>方法入参/出参<br/>分支条件判断结果]
        Q2 -->|否| Q3{"这是业务关键节点<br/>或系统状态变更 ?"}
        Q3 -->|是| INFO[INFO<br/>请求开始/结束<br/>订单状态变更<br/>缓存命中/未命中]
        Q3 -->|否| Q4{"发生了可恢复的异常<br/>或降级处理 ?"}
        Q4 -->|是| WARN[WARN<br/>接口超时重试<br/>限流触发<br/>配置缺失使用默认值]
        Q4 -->|否| Q5{"发生了不可恢复的错误<br/>需要人工介入 ?"}
        Q5 -->|是| ERROR[ERROR<br/>数据库连接失败<br/>第三方接口不可用<br/>业务核心流程中断]
        Q5 -->|否| NOLOG[(不记录日志)]
    end
...
flowchart LR
    ...
    BRIDGE --> B3["logback-classic (适配 Logback<br/>Spring Boot 默认)"]
...
flowchart TD
    ...
    subgraph HIERARCHY ["Logback Logger 三级继承体系"]
        ROOT_LOGGER["🌳 ROOT Logger<br/>级别：INFO<br/>是所有 Logger 的最终祖先<br/>name 为 'ROOT'"]
        ...
        subgraph CLASS ["类级 Logger (继承自包级)"]
            ORDER_SVC["com.example.service.OrderService<br/>级别：null → 继承 com.example.service 的 DEBUG"]
            USER_SVC["com.example.service.UserService<br/>级别：null → 继承 com.example.service 的 DEBUG"]
            ORDER_CTL["com.example.controller.OrderController<br/>级别：null → 继承 ROOT 的 INFO"]
        end
...
flowchart TD
    ...
    subgraph PIPE ["Logback 日志事件处理管道"]
        START([业务代码调用<br/>logger.info]) --> APPENDER_GET["获取 Appender 列表<br/>从 Logger 继承链收集"]

        APPENDER_GET --> FILTER_LEVEL{"级别过滤<br/>日志级别 >= Logger 有效级别 ?"}
        FILTER_LEVEL -->|否| DISCARD1[(丢弃)]
        FILTER_LEVEL -->|是| TURBO_FILTER{"TurboFilter<br/>全局过滤 ?"}

        TURBO_FILTER -->|被拒绝| DISCARD2[(丢弃)]
        TURBO_FILTER -->|通过| CREATE_EVENT["创建 LoggingEvent<br/>封装：消息、参数、级别<br/>时间戳、线程名、MDC"]

        CREATE_EVENT --> APPENDER_LOOP["遍历所有 Appender"]

        APPENDER_LOOP --> APP_FILTER{"Appender 级别过滤<br/>event级别 >= Appender级别 ?"}
        APP_FILTER -->|否| NEXT_APP["下一个 Appender"]
        APP_FILTER -->|是| APP_CUSTOM_FILTER{"自定义 Filter<br/>是否接受 ?"}

        APP_CUSTOM_FILTER -->|DENY| NEXT_APP
        APP_CUSTOM_FILTER -->|ACCEPT/NEUTRAL| ENCODE["Encoder 编码<br/>PatternLayout 将事件<br/>格式化为字符串"]

        ENCODE --> WRITE["Appender 输出<br/>ConsoleAppender → System.out<br/>FileAppender → 文件<br/>RollingFileAppender → 滚动文件"]
...
flowchart LR
    ...
    ROOT --> FACADE[日志门面<br/>应用代码只依赖此层]
    FACADE --> SLF4J[SLF4J<br/>唯一推荐]
    FACADE --> JCL[Jakarta Commons Logging<br/>已过时，不推荐]
    FACADE --> JUL_INTERFACE[JUL 自带接口<br/>功能有限，不推荐]

    ROOT --> IMPL[日志实现<br/>实际处理日志事件]
    IMPL --> LOGBACK[Logback<br/>Spring Boot 默认]
    IMPL --> LOG4J2[Log4j2<br/>Apache 新一代]
    IMPL --> JUL[JUL java.util.logging<br/>JDK 自带，不推荐]
    IMPL --> LOG4J1[Log4j 1.x<br/>2015年停止维护，禁止使用]
...
sequenceDiagram
    ...
    SERVER-->>DEV: -rw-r--r-- app.log 380MB<br/>-rw-r--r-- app.2022-09-30.0.log 100MB
    ...
    SERVER-->>DEV: 189 数据库连接超时<br/>35 库存扣减失败<br/>23 NullPointerException

    DEV->>DEV: 第三步：锁定时间段
    DEV->>SERVER: grep '2022-09-30 10:1[0-5]' app.log | grep 'ERROR'
    SERVER-->>DEV: 10:13 ~ 10:15 期间密集出现<br/>数据库连接超时
...
flowchart TD
    ...
    subgraph PGL ["Prometheus + Grafana + Loki 轻量日志聚合架构"]
        subgraph APPS ["📦 应用实例层"]
            direction LR
            APP1["App Instance 1<br/>日志文件 + /actuator/prometheus"]
            APP2["App Instance 2<br/>日志文件 + /actuator/prometheus"]
            APP3["App Instance N<br/>日志文件 + /actuator/prometheus"]
        end

        subgraph COLLECT ["📥 采集层"]
            direction LR
            PROMTAIL1["Promtail<br/>tail 日志"]
            PROMTAIL2["Promtail<br/>tail 日志"]
            PROMTAIL3["Promtail<br/>tail 日志"]
            PROMETHEUS["Prometheus<br/>每 15s 拉取指标"]
        end

        subgraph STORE ["💾 存储层"]
            LOKI["Loki<br/>标签索引 + 对象存储<br/>• 不对全文建索引<br/>• 按 Stream 压缩存储<br/>• 自动过期删除"]
            PROM_TSDB["Prometheus TSDB<br/>本地时序数据库<br/>• 默认保留 15 天<br/>• 高效压缩"]
        end

        subgraph VIEW ["👁 统一展示与告警层"]
            GRAFANA["Grafana<br/>• LogQL 查日志<br/>• PromQL 查指标<br/>• 日志+指标同屏联动<br/>• 告警规则配置"]
            ALERTMANAGER["AlertManager<br/>• 邮件/钉钉/企微告警<br/>• 告警分组/静默/抑制"]
        end
...
flowchart TD
    ...
    subgraph SUMMARY ["Spring Boot 日志知识总览"]
        direction TB
        WHERE["📍 打点位置<br/>Controller: INFO 请求入口/出口<br/>Service: INFO 状态变更 + DEBUG 分支决策<br/>DAO: 由框架自动输出 SQL"]

        HOW["⚙️ 框架原理<br/>SLF4J: 门面接口<br/>Logback: Logger → Appender → Encoder<br/>三级继承: ROOT → Package → Class"]

        CONFIG["🔧 配置实践<br/>application.yml: 简单场景<br/>logback-spring.xml: 复杂场景<br/>Pattern 含 TraceId + 脱敏"]

        SEARCH["🔍 线上排查<br/>单机: tail + grep + awk<br/>集群: Prometheus + Grafana + Loki<br/>全链路: TraceId + MDC"]

        ANTI["🚫 反模式<br/>循环打 INFO<br/>字符串拼接<br/>吞异常不打日志<br/>敏感信息未脱敏"]
    end
