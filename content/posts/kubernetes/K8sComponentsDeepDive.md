...
- **控制面（control plane）**：集群的大脑——**决策**。存状态、做调度、收敛声明，全在控制面；
- **节点（node）**：集群的肌肉——**执行**。拉镜像、起容器、转发流量、管网络，全在节点。
...
| 组件 | 一句话职责 | 对应命令/现象 |
|------|------|------|
| **kubelet** | 节点上的"经理"：管理分配给本节点的 Pod（创建/销毁容器）、**执行探针**、向 apiserver 上报节点和 Pod 状态 | 探针失败、节点 NotReady → 找它 |
