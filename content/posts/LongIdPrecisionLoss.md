...
flowchart TD
    ...
    JAVA[Java Long<br/>1857353925587607552]
    JSON[JSON 数字<br/>1857353925587607552]
    PARSE[JavaScript JSON.parse]
    NUM[JS Number<br/>1857353925587607500]
    QUERY[用错误ID查数据库]
    MISS[查不到数据]
...
flowchart TD
    ...
    SNOW[雪花算法ID<br/>17 ~ 19位十进制]
    SAFE[JS Number安全整数上限<br/>9007199254740991<br/>约16位]
    UNSAFE[超出安全范围<br/>精度丢失区域]
...
sequenceDiagram
    ...
    Note over J: 写入 1857353925587607552<br/>作为 JSON 数字（无引号）
    J-->>F: {"orderId":1857353925587607552}
    F->>F: JSON.parse()
    Note over F: Number(1857353925587607552)<br/>= 1857353925587607500
```
...
flowchart TD
    ...
    SERIAL -->|Long/long| STR[ToStringSerializer<br/>输出带引号字符串]
    SERIAL -->|BigDecimal| PLAIN[WRITE_BIGDECIMAL_AS_PLAIN<br/>输出纯数值]
    ...
    FRONT --> CORRECT[orderId为字符串<br/>精度零丢失]
