...
spring.cloud.nacos.discovery.group:  ???     # ← 你猜啥？DEFAULT_GROUP
```
      ...
      datasource:
        flow:
          nacos:
            ...
            groupId: mall-cloud
            rule-type: flow
            # ❌ 以为继承了 application.yml 的 namespace 和账号密码
            # 实际上这里每个字段都是独立的，不写就为空
```
...
> ⚠️ 这条不只适用于 Sentinel。任何**从 Nacos 加载的配置中嵌套了另一个 Nacos 连接参数**的场景（比如动态数据源、Seata 配置等），都要手动补全连接信息。
