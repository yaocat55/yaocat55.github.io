...
classDef highlight fill:#450a0a,stroke:#dc2626,stroke-width:1.5px,color:#fecaca,font-weight:bold;
    ...
    ROOT --> B1[认证相关过滤器]
    B1 --> F1["🔐 UsernamePassword\nAuthenticationFilter\n处理用户名密码登录"]
    B1 --> F2["🔑 JwtAuthentication\nFilter 自定义\n从请求头提取JWT\n解析并设置认证状态"]

    ROOT --> B2[授权相关过滤器]
    B2 --> F3["✅ FilterSecurity\nInterceptor\n最终授权决策点\n判断是否有权限访问"]
    B2 --> F4["🛡️ ExceptionTranslation\nFilter\n将认证/授权异常\n转换为HTTP响应"]

    ROOT --> B3[安全基础过滤器]
    B3 --> F5["🧱 SecurityContext\nPersistenceFilter\n安全上下文持久化"]
    B3 --> F6["🚫 CsrfFilter\nCSRF防护\n前后端分离可关闭"]
...
sequenceDiagram
    ...
    participant F2 as JWT认证过滤器\n自定义
    participant F3 as FilterSecurity\nInterceptor
    participant F4 as ExceptionTranslation\nFilter
    participant API as Controller

    C->>F1: HTTP请求
    F1->>F1: 检查SecurityContext\n是否已有认证信息
    F1->>F2: 继续

    alt 请求头带了JWT
        F2->>F2: 1.提取Token\n2.解析JWT获取用户信息\n3.查询用户权限
        F2->>F1: 4.设置Authentication到\nSecurityContextHolder
    else 请求头未带JWT
        F2->>F2: 跳过 匿名访问
    end
...
sequenceDiagram
    ...
    C->>JWT: GET /api/user/info\n无 Authorization Header
    JWT->>JWT: 无Token, 直接放行
    JWT->>SEC: 继续过滤器链
    SEC->>SEC: 检查URL规则\nanyRequest().authenticated()
    SEC->>SEC: SecurityContext中\n无认证信息
