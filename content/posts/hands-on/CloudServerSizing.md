...
classDef dim fill:#2d1a05,stroke:#f59e0b,stroke-width:2px,color:#fde68a,font-weight:bold;
classDef leaf fill:#1e1e24,stroke:#6b7280,stroke-width:1.5px,color:#e5e7eb;
classDef formula fill:#450a0a,stroke:#dc2626,stroke-width:1.5px,color:#fecaca,font-weight:bold;
    ...
    BW --> BW_F["公式: QPS × 响应体大小 × 8\n÷ 1000 ÷ 冗余系数"]
    CPU --> CPU_F["公式: QPS ÷ 单核RPS\n× 安全系数"]
    MEM --> MEM_F["公式: 堆内存 + 非堆\n+ OS开销 + 安全余量"]
...
flowchart TD
classDef startEnd fill:#701a4c,stroke:#e11d48,stroke-width:2px,color:#fce7f3,font-weight:bold;
classDef heap fill:#052e16,stroke:#16a34a,stroke-width:1.5px,color:#bbf7d0,font-weight:bold;
classDef nonheap fill:#0f172a,stroke:#3b82f6,stroke-width:2px,color:#bfdbfe,font-weight:bold;
classDef os fill:#450a0a,stroke:#dc2626,stroke-width:1.5px,color:#fecaca,font-weight:bold;
classDef total fill:#450a0a,stroke:#dc2626,stroke-width:2px,color:#fecaca,font-weight:bold;
    ...
    HEAP --> H1[年轻代 Young Gen\nEden + S0 + S1]
    HEAP --> H2[老年代 Old Gen]
    NONHEAP --> N1[元空间 Metaspace\n类元数据、方法信息]
    NONHEAP --> N2[直接内存 Direct Memory\nNIO Buffer、Netty]
    NONHEAP --> N3[线程栈 Thread Stacks\n线程数 × -Xss]
    NONHEAP --> N4[JIT编译、GC、\nCodeCache等]
    OS_MEM --> O1[操作系统内核\n~0.5~1GB]
    OS_MEM --> O2[其他进程\nMySQL/Redis/Nginx]
    SAFE_MEM --> S1[防止OOM的余量\n总内存的15%~25%]
...
classDef highlight fill:#450a0a,stroke:#dc2626,stroke-width:1.5px,color:#fecaca,font-weight:bold;
    ...
    MERGE --> Q1{是否有特殊需求?}
    Q1 -- CPU密集型 --> C_SERIES[c系列\n计算型实例\n高主频CPU]
    Q1 -- 内存密集型 --> R_SERIES[r系列\n内存型实例\nCPU:内存=1:8]
    Q1 -- 通用场景 --> G_SERIES[g系列\n通用型实例\nCPU:内存=1:4]
    Q1 -- 突发流量 --> T_SERIES[t系列\n突发性能实例\n适合低负载有突发场景]

    C_SERIES --> FINAL[确定最终实例规格\n如 ecs.c7.2xlarge]
    R_SERIES --> FINAL
    G_SERIES --> FINAL
    T_SERIES --> FINAL
