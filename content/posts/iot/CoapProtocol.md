...
sequenceDiagram
    ...
    Note over C: ACK 中携带了业务数据<br/>一个往返就完成了请求+响应
...
flowchart TD
    ...
    START([固件 OTA 开始]) --> REQ_BLOCK["客户端请求 Block2 Option<br>表示支持分块接收"]

    REQ_BLOCK --> SVR_CHUNK["服务器返回第一块<br>携带 Block2 Option<br>NUM=0 | M=1(还有更多) | SZX=1024"]

    SVR_CHUNK --> LOOP_CLIENT{客户端检查<br>Block2.M 标志}

    LOOP_CLIENT -- M=1 (还有更多块) --> REQ_NEXT["客户端请求下一个 Block<br>Block2 NUM 递增"]
    REQ_NEXT --> SVR_NEXT["服务器返回下一块"]
    SVR_NEXT --> LOOP_CLIENT

    LOOP_CLIENT -- M=0 (最后一块) --> DONE([分块传输完成<br>客户端重组完整文件])
