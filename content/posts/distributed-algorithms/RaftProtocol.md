...
flowchart TD
    ...
    subgraph majority_principle["多数派原理"]
        M1["3个节点——多数 = 2"]:::data
        M2["5个节点——多数 = 3"]:::data
        M3["任何两次多数投票<br/>必然有至少一个<br/>节点重叠"]:::highlight
    end

    M1 --> M3
    M2 --> M3

    M3 --> result["这个重叠节点<br/>记住了上次的投票结果<br/>→ 不可能出现<br/>两个冲突的多数决定"]:::condition

    result --> why["这就是 Raft 集群<br/>必须是奇数的原因<br/>偶数会出现平局"]:::data
```
...
stateDiagram-v2
    ...
    Follower --> Candidate: 选举超时<br/>没收到Leader心跳
    Candidate --> Leader: 收到多数投票

    Candidate --> Follower: 发现更高任期<br/>或选举超时分票
...
sequenceDiagram
    ...
    Note over N1,N3: 初始——三个节点都是Follower<br/>Leader刚宕机

    rect rgb(255, 243, 224)
        Note over N2: 节点B的选举超时先到<br/>(150ms~300ms随机)
        N2->>N2: 任期号+1→变成Candidate<br/>先投自己一票
        N2->>N1: 请求投票 term=5
        N2->>N3: 请求投票 term=5
    end
...
flowchart TD
    ...
    write --> step1["① Leader追加到本地日志<br/>状态: uncommitted"]:::process
    step1 --> step2["② 并发发送AppendEntries<br/>给所有Follower"]:::process
    step2 --> check{"③ 多少Follower确认？"}:::condition
    check -->|"≥ 多数(含Leader)"| commit["④ Leader标记committed<br/>应用到状态机<br/>✅ 回复客户端成功"]:::data
    check -->|"＜ 多数"| fail["❌ 写入失败<br/>可能触发重新选举"]:::reject

    commit --> heartbeat["⑤ 下次心跳告知Follower<br/>该日志已提交——Follower也应用"]:::process
```
...
flowchart TD
    ...
    normal["三节点集群<br/>A B C 互通"]:::data

    normal --> partition["⚡ 网络分区"]:::highlight

    partition --> majority["分区1: A + B<br/>节点数=2 ≥ 多数<br/>✅ 能选举Leader<br/>✅ 能正常写入"]:::data
    partition --> minority["分区2: C<br/>节点数=1 ＜ 多数<br/>❌ 不能选举<br/>❌ 不能写入"]:::reject

    majority --> result["保证了:<br/>任何时刻最多只有一个<br/>能工作的多数派<br/>→ 数据不会出现<br/>两份冲突的版本"]:::condition

    minority -.-> recover["网络恢复后<br/>C从Leader同步<br/>追平数据"]:::process
    majority -.-> recover
```
...
flowchart TD
    ...
    Q["Raft解决什么问题"]:::process --> Q1["多台机器需要对<br/>数据的值达成一致<br/>且不能出错"]:::process
    Q --> Q2["Paxos太难实现<br/>工程界需要<br/>一个能看懂的方案"]:::process

    Q1 --> A1["领导者选举<br/>随机超时+多数投票<br/>保证任何时刻最多<br/>一个Leader"]:::data
    Q2 --> A2["日志复制<br/>Leader写入→多数确认<br/>→标记committed<br/>少数派不能独立提交"]:::data

    A1 --> A3["任期号<br/>单调递增——发现<br/>更高任期自动降级<br/>防止脑裂"]:::data
    A2 --> A4["安全规则<br/>日志不够新不能当选<br/>只提交当前任期日志<br/>防止已提交数据丢失"]:::data

    A3 --> R["结果: CP模型<br/>网络分区时优先一致<br/>少数派不可用<br/>分区恢复后自动追平"]:::data
    A4 --> R

    R --> USE["Nacos配置中心走Raft<br/>Nacos服务发现走Distro<br/>同一套系统——两种协议<br/>各取所需"]:::process
```
