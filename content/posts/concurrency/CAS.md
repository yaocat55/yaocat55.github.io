...
sequenceDiagram
    ...
    Note over CA,CB: 初始状态：缓存行在 A 和 B 中都为 S（Shared）\ncount = 0 存在于两个 L1 中
...
classDef data fill:#052e16,stroke:#16a34a,stroke-width:2px,color:#bbf7d0,font-weight:bold;
    ...
    subgraph CORE_B [Core B]
        REG_B["寄存器: count=1\n基于旧的count=0"]
        L1_B["L1 Cache\ncount 缓存行: S 还没收到Invalidate"]
        IQ_B["Invalidate Queue\n待处理: Invalidate count"]
    end
    BUS["总线"]
...
class IQ_B,L1_B condition;
class L1_A,SB_A data;
class BUS,REG_A,REG_B process;
```
...
sequenceDiagram
    ...
    Note over CA,CB: count = 0, 两个核心都尝试 CAS(0, 1)\n期望旧值=0, 新值=1

    CA->>BUS: lock cmpxchg [addr], 1\n（原子：比较+交换）
    Note over BUS: lock 前缀锁定缓存行\n阻止其他核心在此期间\n访问同一缓存行
...
classDef data fill:#052e16,stroke:#16a34a,stroke-width:2px,color:#bbf7d0,font-weight:bold;
classDef condition fill:#2a1147,stroke:#a855f7,stroke-width:2px,color:#ede9fe,font-weight:bold;
    LOCK["lock 前缀指令"] --> CHECK{"操作的数据\n是否在一条缓存行内？"}
    CHECK -->|"是（绝大多数情况）"| CACHE_LOCK["缓存行锁\n（Cache Lock）"]
    CHECK -->|"否（跨缓存行 / 不可缓存）"| BUS_LOCK["总线锁\n（Bus Lock）"]

    CACHE_LOCK --> MESI_LOCK["缓存行锁：\n发送 RFO → 置为 M 状态\n失效其他核心副本"]
    BUS_LOCK --> BUS_SIGNAL["总线锁：\nLOCK 引脚锁定总线\n阻止所有核心访问内存"]

    MESI_LOCK --> COMPLETE["锁仅在指令执行期间有效\n（几个 CPU 周期）"]
    BUS_SIGNAL --> COMPLETE

class CHECK condition;
class CACHE_LOCK,MESI_LOCK data;
class BUS_SIGNAL highlight;
class BUS_LOCK,COMPLETE,LOCK process;
```
...
classDef startEnd fill:#701a4c,stroke:#e11d48,stroke-width:2.5px,color:#fce7f3,font-weight:bold;
    root((java.util.concurrent.atomic\n12个原子类))
...
class AB,AI,AIA,AIFU,AL,ALA,ALFU,AMR,AR,ARA,ARFU,ARRAY,ASR,BASIC,REF,UPDATER process;
class root startEnd;
```
...
class CACHE,MESI data;
class AI,ATOMIC,CPU,NATIVE,U process;
class cmpxchg,compareAndSwapInt,incrementAndGet startEnd;
```
