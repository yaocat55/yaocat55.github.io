...
1970 年代，CPU 直接从内存读数据，内存足够快。到了 1990 年代，CPU 主频飙到几百 MHz，内存还是几十 ns 的访问延迟——一颗 200MHz 的 CPU，等一次内存读取等于浪费十几个指令周期。CPU 架构师的应对方案是加缓存：把热数据放在离核心最近的地方。

缓存解决了速度问题，但制造了一个新问题：**多核 CPU 中，同一个内存地址可能在多个核心的私有缓存中各有副本。** Core A 修改了 `count = 1`，Core B 的缓存中 `count` 还是 0——Core B 基于过期数据继续算，结果全错。

这个问题的本质不是"哪个值更正确"，而是**各个核心对同一地址的数据要有统一的认知**——这就是缓存一致性（Cache Coherence）。没有它，多核处理器就等于多个单核处理器各算各的，合不起来。
...
class CL,DATA,ST data;
class DB,DIR,L,VB process;
```
...
sequenceDiagram
    participant B as Core B
    participant BUS as 总线
    participant IQ_A as Core A\nInvalidate Queue
    participant L1_A as Core A\nL1 Cache
    participant A as Core A

    B->>B: write x=1\n缓存行: S → M
    B->>BUS: BusUpgr(x)
    BUS->>IQ_A: Invalidate(x)
    Note over IQ_A: 放入 Invalidate Queue\n立即回 ACK
    IQ_A->>BUS: ACK
    BUS->>B: 所有 ACK 收齐\n写入完成

    A->>L1_A: read x
    L1_A->>L1_A: 缓存行仍是 S 状态！
    Note over A: 读到旧值 x=0\n（Invalidate Queue 还没处理）
...
sequenceDiagram
    ...
    CA->>CA: Thread A 写 varA\n缓存行: E → M
    CB->>CB: Thread B 想写 varB\n缓存行: I（已被 Core A 失效）
    CB->>BUS: BusRdX（带数据请求）
    BUS->>CA: Invalidate
    CA->>CA: 缓存行: M → I
    CA->>BUS: 提供数据 + ACK
    BUS->>CB: 数据
    CB->>CB: 缓存行: I → M\nThread B 写入 varB
    Note over CA,CB: 一轮 ping-pong 完成\n双方各经历一次缓存缺失

    CA->>CA: Thread A 再次写 varA\n缓存行: I（又被 Core B 失效了）
    CA->>BUS: BusRdX（又一次！）
    BUS->>CB: Invalidate（又一次！）
    CB->>CB: 缓存行: M → I
    Note over CA,CB: 持续循环——每次写都是缓存缺失\n性能退化到接近主内存级别
```
...
classDef startEnd fill:#701a4c,stroke:#e11d48,stroke-width:2.5px,color:#fce7f3,font-weight:bold;
    ...
    C --> C1[Store Buffer\n写操作异步化]
    C --> C2[Invalidate Queue\n失效消息异步化]
    C --> C3[Store Forwarding\n同核心写入立即可见]
...
class C2 condition;
class A,B,C1,C3,F1,F2 data;
class B1,B2,B3,C,D,D1,D2,D3,E,F,G process;
class H startEnd;
```
