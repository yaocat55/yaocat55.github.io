...
classDef reject fill:#450a0a,stroke:#dc2626,stroke-width:2px,color:#fecaca,font-weight:bold;
    ...
    H -->|抛出异常| J[setException\nNEW→COMPLETING→EXCEPTIONAL]
    I --> K[finishCompletion\n唤醒所有等待线程]
    J --> K
    K --> F
    F -->|runner=null| L{state >= INTERRUPTING?}
    L -->|是| M[handlePossibleCancellationInterrupt\n自旋等待中断完成]
    L -->|否| N[结束]
    M --> N

class B,E,H,L condition;
class I data;
class D,F,G process;
class J reject;
class A,C,K,M,N startEnd;
```
...
classDef branch fill:#2d1a05,stroke:#f59e0b,stroke-width:2px,color:#fde68a,font-weight:bold;
    ...
    F1 --> B
    F -->|是| G{设置了超时?}
    G -->|否| G1[LockSupport.park\n无限阻塞]
    G1 --> B
    G -->|是| H{已超时?}
    H -->|是| H1[removeWaiter\n返回当前 state]
    H -->|否| H2[LockSupport.parkNanos\n定时阻塞]
    H2 --> B

class E1 branch;
class B,C,D,E,F,G,H condition;
class D1,F1,G1,H2 process;
class B1 reject;
class A,C1,H1 startEnd;
```
...
class C1,C2,C5,CFUTURE,F1,F2,F3,FUTASK process;
class C3 reject;
class C4 startEnd;
```
...
class I branch;
class B,L,WAIT data;
class A,C,D,E,EXEC,F,G,H,IDENTITY,J,K,STATE process;
class M reject;
class N startEnd;
```
