...
flowchart LR
    root((Semaphore))
    root --> SYNC[内部类 Sync<br/>extends AQS]
    SYNC --> NS[NonfairSync<br/>非公平同步器]
    SYNC --> FS[FairSync<br/>公平同步器]
    SYNC --> STATE[state 字段<br/>当前可用许可数]
    SYNC --> QUEUE[CLH 同步队列<br/>Node.SHARED 节点]
    NS --> NS_TRY["tryAcquireShared()<br/>直接 CAS 抢占"]
    FS --> FS_TRY["tryAcquireShared()<br/>先 hasQueuedPredecessors()"]
```
...
```mermaid
flowchart TD
    HEAD[head 哑结点<br/>thread=null<br/>waitStatus=SIGNAL]
    N1["Node-SHARED<br/>thread=T1<br/>waitStatus=SIGNAL"]
    N2["Node-SHARED<br/>thread=T2<br/>waitStatus=0"]
    N3["Node-SHARED<br/>thread=T3<br/>waitStatus=0"]
    TAIL[tail]
    HEAD -->|next| N1
    N1 -->|prev| HEAD
    N1 -->|next| N2
...
flowchart TD
    ...
    F --> G{head != null<br/>且 head != tail?}
    G -->|否| H[队列空，无需唤醒]
    G -->|是| I{head.waitStatus?}
    I -->|SIGNAL| J[CAS 清 waitStatus<br/>unparkSuccessor head]
...
```mermaid
flowchart TD
    root((AQS 同步器<br/>四种典型实现))
    root --> SM[Semaphore<br/>共享模式]
    root --> CDL[CountDownLatch<br/>共享模式]
    root --> CB[CyclicBarrier<br/>独占锁+Condition]
    root --> RL[ReentrantLock<br/>独占模式]
    SM --> SM_D["state=许可数<br/>可增可减<br/>支持公平策略<br/>不支持Condition<br/>可重用"]
    CDL --> CDL_D["state=倒计数<br/>只减不增<br/>不支持公平<br/>不支持Condition<br/>一次性"]
    CB --> CB_D["通过ReentrantLock实现<br/>可循环重置<br/>支持回调<br/>批量唤醒"]
    RL --> RL_D["state=重入次数<br/>持有者记录<br/>支持公平策略<br/>支持Condition<br/>可重入"]
```
...
flowchart TD
    Q1{需要做什么?}
    Q1 -->|"控制同时访问<br/>资源的线程数"| SM[选 Semaphore]
    Q1 -->|"等待 N 个任务<br/>全部完成"| CDL[选 CountDownLatch]
    Q1 -->|"多线程互相等待<br/>然后同时执行"| CB[选 CyclicBarrier]
    Q1 -->|"互斥保护<br/>临界区代码"| Q2{需要哪些特性?}
    Q2 -->|"可中断/超时/<br/>公平锁/多Condition"| RL[选 ReentrantLock]
...
flowchart TD
    ...
    SYNC --> STATE[volatile int state<br/>当前可用许可数<br/>无上限约束]
    SYNC --> MODE[SHARED 共享模式]
    MODE --> ACQUIRE[acquireSharedInterruptibly<br/>tryAcquireShared<br/>返回剩余许可数]
    MODE --> RELEASE[releaseShared<br/>tryReleaseShared<br/>state+1 CAS]
    MODE --> PROP[setHeadAndPropagate<br/>链式传播唤醒]
    SYNC --> FAIR[公平策略]
    FAIR --> NF[NonfairSync<br/>直接CAS抢占]
    FAIR --> FF[FairSync<br/>hasQueuedPredecessors检查]
    SYNC --> TOOLS[辅助方法]
    TOOLS --> DRAIN[drainPermits<br/>一次性清零]
    TOOLS --> REDUCE[reducePermits<br/>永久缩减配额]
```
