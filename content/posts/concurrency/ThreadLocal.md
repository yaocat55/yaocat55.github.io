...
```mermaid
flowchart TD
    classDef thread fill:#F48FB1,stroke:#C2185B,stroke-width:2px,color:#212121,font-weight:bold;
    classDef map fill:#FFE082,stroke:#FFB300,stroke-width:2px,color:#5D4037,font-weight:bold;
    classDef entry fill:#C8E6C9,stroke:#388E3C,stroke-width:1.5px,color:#1B5E20,font-weight:bold;
    classDef tl fill:#E1BEE7,stroke:#7B1FA2,stroke-width:1.5px,color:#212121,font-weight:bold;
    classDef val fill:#F5F5F5,stroke:#9E9E9E,stroke-width:1.5px,color:#212121;

    subgraph THREAD_S["每个Thread对象"]
        TA[Thread A<br/>threadLocals字段]
        TB[Thread B<br/>threadLocals字段]
    end
    ...
    subgraph GLOBAL["全局ThreadLocal实例"]
        TL_A["ThreadLocal A<br/>(如SDF_HOLDER)"]
        TL_B["ThreadLocal B<br/>(如USER_HOLDER)"]
    end
    subgraph DATA_S["实际存储的数据"]
        V_A["SimpleDateFormat实例<br/>线程A的副本"]
        V_B["User实例<br/>线程A的副本"]
    end
...
sequenceDiagram
    ...
    Note over WT: init()中复制父线程的<br/>inheritableThreadLocals<br/>子线程拿到 traceId-001 ✅
    ...
    MT->>TP: submit(task2)
    TP->>WT: 复用已有工作线程
    Note over WT: 没有 new Thread()<br/>没有 Thread.init()<br/>ITL 值仍是 "traceId-001" ❌
    WT->>WT: task2.run() → ITL.get() = "traceId-001"
    Note over WT: 拿到了上一次的值！<br/>主线程设置的 "traceId-002" 丢失
...
flowchart TD
    ...
    classDef defect fill:#FFCDD2,stroke:#C62828,stroke-width:1.5px,color:#B71C1C,font-weight:bold;
    ...
    D1 --> D1_SRC["传递绑定在Thread.init()方法<br/>new Thread()时触发一次<br/>线程池复用线程不经过init()"]
    D2 --> D2_SRC["childValue()默认return parentValue<br/>直接返回原对象引用<br/>父子线程共享同一对象"]
    D3 --> D3_SRC["ThreadLocalMap(parentMap)执行完后<br/>父子Map是两个独立Entry[]数组<br/>无任何同步机制"]
...
sequenceDiagram
    ...
    Note over TR: 快照当前线程所有TTL值<br/>{traceId: "traceId-001"}
...
```mermaid
flowchart TD
    classDef itl fill:#FFCDD2,stroke:#C62828,stroke-width:1.5px,color:#B71C1C,font-weight:bold;
    classDef ttl fill:#C8E6C9,stroke:#388E3C,stroke-width:1.5px,color:#1B5E20,font-weight:bold;
    classDef common fill:#F5F5F5,stroke:#9E9E9E,stroke-width:1.5px,color:#212121;
    ...
    ITL_PATH --> CHECK1{"线程池是否<br/>已有线程?"}
    CHECK1 -- 是(复用) --> SKIP_INIT["不经过Thread.init()"]
    SKIP_INIT --> ITL_FAIL["❌ traceId丢失<br/>拿到旧值或null"]
    CHECK1 -- 否(新建) --> INIT["Thread.init()复制"]
    INIT --> ITL_OK["✓ 拿到traceId<br/>但仅这一次"]

    TTL_PATH --> WRAP["TtlRunnable.get()"]
    WRAP --> CAPTURE["Transmitter.capture()<br/>快照当前线程所有TTL值"]
    CAPTURE --> QUEUE["包装后的Runnable入队列"]
    QUEUE --> EXECUTE["工作线程取出任务"]
    EXECUTE --> REPLAY["Transmitter.replay()<br/>将快照设置到工作线程"]
    REPLAY --> EXEC["run()执行业务逻辑"]
    EXEC --> RESTORE["Transmitter.restore()<br/>恢复工作线程原值"]
    RESTORE --> TTL_OK["✓ 每次都能拿到<br/>正确的traceId"]
...
```mermaid
flowchart TD
    classDef base fill:#F5F5F5,stroke:#9E9E9E,stroke-width:1.5px,color:#212121;
    classDef itl fill:#FFE082,stroke:#FFB300,stroke-width:2px,color:#5D4037,font-weight:bold;
    classDef ttl fill:#C8E6C9,stroke:#388E3C,stroke-width:2px,color:#1B5E20,font-weight:bold;
    classDef field fill:#E1BEE7,stroke:#7B1FA2,stroke-width:1.5px,color:#212121;

    TL["ThreadLocal<br/>数据路由到<br/>threadLocals"] --> ITL["InheritableThreadLocal<br/>重写getMap/createMap<br/>数据路由到<br/>inheritableThreadLocals"]
    ITL --> TTL_CLASS["TransmittableThreadLocal<br/>继承ITL<br/>数据仍然路由到<br/>inheritableThreadLocals<br/>新增:holder注册+capture/replay/restore"]
...
flowchart TD
    ...
    CORE --> TL_F["线程隔离<br/>每个线程独立Map<br/>存储于threadLocals"]
    CORE --> REF["弱引用Key<br/>自清理机制<br/>必须remove保证不泄漏"]

    ITL_CORE[InheritableThreadLocal<br/>继承ThreadLocal]
    ITL_CORE --> ITL_ADD["新增能力<br/>Thread.init()中复制<br/>父到子的单向传递"]
    ITL_CORE --> ITL_BUG["三大缺陷<br/>1.线程池复用失效<br/>2.childValue浅拷贝<br/>3.创建后不再同步"]

    TTL_CORE[TransmittableThreadLocal<br/>继承InheritableThreadLocal]
    TTL_CORE --> TTL_FIX["解决ITL缺陷<br/>capture在submit时触发<br/>replay在run前执行<br/>restore在run后恢复"]
    TTL_CORE --> TTL_DESIGN["设计要点<br/>holder全局注册<br/>装饰器模式包装<br/>三段式操作保证传递"]
