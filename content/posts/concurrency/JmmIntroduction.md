...
class CACHE,EXCL,HARDWARE data;
class HB,INVAL,JMM_LAYER,MM,ORDER,SYNC,VOL process;
```
...
sequenceDiagram
    ...
    JMM->>JMM: 将工作内存中 x 的副本\n强制刷新到主内存
    Note over JMM: 对标 MESI:\nM 状态核心将脏数据写回\n+ 失效其他缓存行
...
class BUS,MESI独占 data;
class CRITICAL,E,ENTER,EXIT,M,SYNC独占 process;
```
...
classDef process fill:#1e1e24,stroke:#6b7280,stroke-width:2px,color:#e5e7eb;
    subgraph HB_RULES[happens-before 规则集]
        R1[程序次序:\n同一线程中,前面的操作\nhappens-before 后面的操作]
        R2[锁规则:\nunlock happens-before\n后续的 lock]
        R3[volatile规则:\nvolatile 写 happens-before\n后续 volatile 读]
        R4[传递性:\n若 A hb B, B hb C,\n则 A hb C]
    end

class HB_RULES,R1,R2,R3,R4 process;
```
