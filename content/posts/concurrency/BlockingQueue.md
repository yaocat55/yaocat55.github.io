...
class ENTRY,INSERT,PARK,TIMED_PARK process;
class THROW reject;
class RETURN_FALSE,SUCCESS startEnd;
```
...
classDef startEnd fill:#701a4c,stroke:#e11d48,stroke-width:2.5px,color:#fce7f3,font-weight:bold;
    ...
    DEQUE --> DEQUE_S[单锁+双Condition\n双向链表]

class ARR_S,DEQUE_S condition;
class ARR,DELAY,DEL_S,DEQUE,LINK,PRI,PRI_S,SYNC,SYNC_S,TRANSFER data;
class LINK_S,TRANS_S process;
class root startEnd;
```
...
class HEAD,N1,N2,N3,NULL_TAIL branch;
class LINK_STRUCT data;
class LOCKS,PL,TL process;
```
...
classDef data fill:#052e16,stroke:#16a34a,stroke-width:2px,color:#bbf7d0,font-weight:bold;
    ...
    subgraph THREAD_POOL[线程池集成]
        T1[FixedPool → LinkedBlockingQueue\n无界排队，需注意OOM]
        T2[CachedPool → SynchronousQueue\n直接交付，动态伸缩]
        T3[ScheduledPool → DelayWorkQueue\n按延迟排序]
    end
    INTERFACE --> LOCK_STRATEGY
    LOCK_STRATEGY --> THREAD_POOL

class L1 condition;
class L2,L3,T1,T2,T3,THREAD_POOL data;
class I2,I3,INTERFACE,LOCK_STRATEGY process;
class I1 reject;
```
