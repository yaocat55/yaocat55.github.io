...
classDef process fill:#1e1e24,stroke:#6b7280,stroke-width:2px,color:#e5e7eb;
    ...
    T1 -->|push/pop\nLIFO 本地| T1Q[Task1 Task2 Task3]
    T2 -->|空闲 steal\nFIFO 窃取| T2Q[Task4 Task5]
...
class STA,T1,T2,T3,T4 data;
class T1Q,T2Q,T3Q,T4Q process;
```
...
classDef process fill:#1e1e24,stroke:#6b7280,stroke-width:2px,color:#e5e7eb;
    ...
    POOL -->|管理| WORKER
    WORKER -->|持有| WQ[WorkQueue\n双端任务队列]
    WORKER -->|执行| TASK
    TASK --> RT
    TASK --> RA
    TASK --> CC

class POOL,WORKER,WQ data;
class TASK process;
class CC,RA,RT startEnd;
```
...
classDef data fill:#052e16,stroke:#16a34a,stroke-width:2px,color:#bbf7d0,font-weight:bold;
    ...
    A --> A1["活跃线程数 - 并行度\nAC = (activeCount - parallelism)"]
    B --> B1["总线程数 - 并行度\nTC = (totalCount - parallelism)"]
    C --> C1["栈顶版本+状态\nstackPred的低16位"]
    D --> D1["栈顶WorkQueue索引\npoolIndex"]

class D1 data;
class A,A1,B,B1,C,C1,CTL,D process;
```
...
class BASE,LOCAL,STEAL,TOP condition;
class WQ data;
class E0,E1,E2,OPS process;
```
...
classDef startEnd fill:#701a4c,stroke:#e11d48,stroke-width:2.5px,color:#fce7f3,font-weight:bold;
    ...
    B --> C[CAS锁定随机偶数槽位]
    C --> D{锁定成功且队列空间充足?}
    D -->|是| E[putOrderedObject\n将task放入数组]
    E --> F[putOrderedInt\n更新top]
    F --> G[putOrderedInt\n解锁qlock]
    G --> H{队列原本为空?}
    H -->|是| I[signalWork\n激活工作线程]
    H -->|否| J[提交完成]
    D -->|否| K[externalSubmit\n完整提交路径]

class D,H condition;
class A,B,C,E,F,G,I,K process;
class J startEnd;
```
...
classDef process fill:#1e1e24,stroke:#6b7280,stroke-width:2px,color:#e5e7eb;
classDef condition fill:#2a1147,stroke:#a855f7,stroke-width:2px,color:#ede9fe,font-weight:bold;
    A[runWorker 开始] --> B[growArray\n分配任务数组]
    B --> C[scan 扫描窃取任务]
    C --> D{scan 成功?}
    D -->|是 有任务| E[runTask 执行任务\ndoExec + execLocalTasks]
    D -->|否 无任务| F{awaitWork 等待}
    F -->|被唤醒| C
    F -->|超时/终止| G[deregisterWorker\n线程退出]
    E -->|更新随机种子| C
...
classDef process fill:#1e1e24,stroke:#6b7280,stroke-width:2px,color:#e5e7eb;
    ...
    N -->|是| O{线程活跃ss>=0?}
    O -->|是| P[return null\n无任务可窃取]
    O -->|否-已失活| Q[设置INACTIVE\nCAS更新ctl减少AC]
    Q --> R[灭活后再次扫描一圈\n双圈检查]
    R -->|仍无任务| S[return null\n进入awaitWork]

class C,E,F,H,K,M,N,O,R condition;
class D,G,I,L,Q process;
class A,B,J,P,S startEnd;
```
...
class FJP,SHARED,T4Q,T5Q,T6Q,TPE data;
...
class A,C,DESIGN,F,H,I process;
class E startEnd;
```
