...
flowchart TD
    ...
    SPLIT -->|链表| LOW_HIGH[低位链放nextTab i<br/>高位链放nextTab i+n]
    SPLIT -->|红黑树| TREE_SPLIT[高低位拆分后<br/>节点<=6则退化为链表]
...
flowchart LR
    ...
    V7 --> SEG[Segment K V 数组<br/>继承ReentrantLock<br/>默认16段,不可扩容]
    SEG --> HASH7[HashEntry K V 数组+链表<br/>每个Segment内部独立扩容]
    V8 --> NODE[Node K V 数组<br/>volatile保证可见性]
    NODE --> HASH8[链表+红黑树<br/>桶级synchronized<br/>多线程协同扩容]
```
