...
class IQ_A,IQ_B condition;
class L1_A,L1_B,L2_A,L2_B,L3,SB_A,SB_B data;
class CORE_A,CORE_B highlight;
class BUS,MEM,REG_A,REG_B process;
```
...
sequenceDiagram
    ...
    CoreB->>SB: stopped = true\n写入 Store Buffer
    Note over CoreB: Core B 继续执行\n后续指令，不等待
    Note over CoreA: Core A 的 L1 中\nstopped = false
    Note over CoreA: 因为写入还在\nStore Buffer 中，\n没有到达 L1 Cache
    SB-->>Bus: 异步刷到 L1\n+ 发送失效信号
...
sequenceDiagram
    participant Bus as 总线
    participant IQ as Invalidate Queue\n(Core A)
    participant L1A as Core A L1

    Bus->>IQ: 发送失效信号\nstopped 的缓存行
    IQ->>Bus: 立即回复 ACK
    Note over L1A: 缓存行仍然是 S 状态\nstopped = false
    Note over IQ: 失效信号在队列中\n等待被处理
```
...
sequenceDiagram
    participant CoreB as Core B
    participant SB_B as Store Buffer\n(Core B)
    participant Bus as 总线
    participant IQ_A as Invalidate Queue\n(Core A)
    participant CoreA as Core A

    CoreB->>SB_B: stopped = true\n（volatile 写）
    Note over CoreB: JVM 在此插入\nStoreLoad 屏障
    CoreB->>SB_B: 屏障强制清空 Store Buffer
    SB_B->>Bus: 写入刷到 L1 Cache\n+ 发送失效信号
    Bus->>IQ_A: Invalidate
    Note over CoreA: JVM 在 volatile 读之前\n插入 LoadLoad 屏障
    CoreA->>IQ_A: 屏障强制处理完\nInvalidate Queue
    CoreA->>CoreA: 缓存行变为 I 状态\n读触发缓存缺失\n获取 stopped = true
```
...
class S1,S2 data;
class AFTER,BEFORE,L1,L2 process;
```
...
class LS,SL,SS data;
class LL,NORMAL_READ,NORMAL_WRITE,VOL_READ,VOL_WRITE,VR,VW process;
```
...
sequenceDiagram
    ...
    participant HW_A as Core A 硬件\n(含 Store Buffer)
    participant Bus as 总线 / 互联
    participant HW_B as Core B 硬件\n(含 Invalidate Queue)
    participant TB as 线程 B\n(Java)

    TA->>HW_A: volatile write x = 1\nJIT: StoreStore + mov + StoreLoad
    Note over HW_A: 清空 Store Buffer\n将 x = 1 刷入 L1 Cache
    HW_A->>Bus: 发送 Invalidate 信号
    Bus->>HW_B: Invalidate 到达
    Note over HW_B: 放入 Invalidate Queue
    TB->>HW_B: volatile read x\nJIT: LoadLoad + mov
    Note over HW_B: 处理完 Invalidate Queue\n缓存行变为 I 状态
    Note over HW_B: 缓存缺失\n从总线获取 x = 1
    HW_B->>TB: x = 1
```
